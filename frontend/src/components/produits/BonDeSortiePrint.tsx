...
interface NewSortieProduitData {
  ...
  detenteur: string; // Ce champ contiendra le numero_matricule
  numero_porte: string;
...
export default function ModalformSorties({
  ...
  const [numeroMatricule, setNumeroMatricule] = useState<string>(''); // État pour le numéro matricule
  const [detenteurNom, setDetenteurNom] = useState<string>(''); // État pour le nom du détenteur (affiché)
  const [detenteurPrenom, setDetenteurPrenom] = useState<string>(''); // État pour le prénom du détenteur (affiché)
  ...
  // Gère l'événement lorsque l'utilisateur quitte le champ numero_matricule
  const handleMatriculeBlur = async () => {
    const matricule = numeroMatricule.trim();
    if (matricule === '') {
      setDetenteurNom('');
      setDetenteurPrenom('');
      setEmployeeError(null); // Clear previous errors
      return;
    }

    setIsLoadingEmployee(true);
    setEmployeeError(null); // Réinitialise l'erreur

    try {
      // Remplacez '/api/employee-details' par l'URL de votre endpoint Node.js
      const response = await fetch(`/api/employee-details/${matricule}`);

      if (!response.ok) {
        // Gérer les erreurs HTTP (par exemple, 404 si non trouvé)
        if (response.status === 404) {
          setEmployeeError('Numéro matricule non trouvé.');
          setDetenteurNom('');
          setDetenteurPrenom('');
        } else {
          setEmployeeError(`Erreur lors de la récupération des détails : ${response.statusText}`);
        }
      ...
      setDetenteurNom(data.nom);
      setDetenteurPrenom(data.prenom);

    } catch (err) {
      console.error('Erreur lors de la récupération des détails de l\'employé:', err);
      setEmployeeError('Erreur réseau ou serveur.');
      setDetenteurNom('');
      setDetenteurPrenom('');
    } finally {
      setIsLoadingEmployee(false);
  ...
  const handleSubmitSortie = async (e: FormEvent) => {
    e.preventDefault();
    // Validation de base
    // detenteur correspond maintenant au numeroMatricule
    if (!produitId || !quantiteSortie || !numeroMatricule || !detenteurNom || !detenteurPrenom || !numeroPorte || Number(quantiteSortie) <= 0) {
        ...
        alert("Veuillez remplir tous les champs obligatoires et vérifier le numéro matricule.");
        return;
    }
    ...
    try {
      const newSortieData: NewSortieProduitData = {
        ...
        detenteur: numeroMatricule, // Envoyer le numero_matricule comme detenteur
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      setQuantiteSortie('');
      setNumeroMatricule(''); // Réinitialiser le matricule
      setDetenteurNom(''); // Réinitialiser le nom
      setDetenteurPrenom(''); // Réinitialiser le prenom
      setNumeroPorte('');
      setObservation('');
      setEmployeeError(null); // Réinitialiser l'erreur employé
    }
  ...
  return (
    ...
    <Dropdown
      ...
      >
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-100 dark:border-700" >
      ...
      <form onSubmit={handleSubmitSortie}>
        ...
        <div>
          <Label htmlFor="numeroMatricule">Numéro Matricule Détenteur</Label> {/* Label pour le champ matricule */}
          <Input
            type="text"
            id="numeroMatricule" // ID pour le champ matricule
            placeholder="Saisir le numéro matricule"
            value={numeroMatricule}
            onChange={(e: ChangeEvent<HTMLInputElement>) => setNumeroMatricule(e.target.value)}
            onBlur={handleMatriculeBlur} // Déclenche la recherche lorsque le champ perd le focus
          />
          {isLoadingEmployee && <p className="text-sm text-gray-500">Chargement...</p>}
          {employeeError && <p className="text-sm text-red-500">{employeeError}</p>}
        </div>
        <div>
          <Label htmlFor="detenteurNom">Nom Détenteur</Label> {/* Label pour le nom affiché */}
          <Input
            type="text"
            id="detenteurNom" // ID pour le nom
            value={detenteurNom}
            readOnly // Ce champ est en lecture seule
            className="bg-gray-100 cursor-not-allowed" // Style pour indiquer qu'il est en lecture seule
          />
        </div>
        <div>
          <Label htmlFor="detenteurPrenom">Prénom Détenteur</Label> {/* Label pour le prénom affiché */}
          <Input
            type="text"
            id="detenteurPrenom" // ID pour le prénom
            value={detenteurPrenom}
            readOnly // Ce champ est en lecture seule
            className="bg-gray-100 cursor-not-allowed" // Style pour indiquer qu'il est en lecture seule
          />
        </div>
